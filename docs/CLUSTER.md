...
and `doc-converter` as sidecars in one Pod: they have different startup times,
resource needs, failure modes, and scaling rules.
...
GPU scheduling is unnecessary for the current CPU pipeline.
...
Ingress path prefixes. Either configure Ingress rewrite rules carefully or use
...
- Run all three IDAR containers as non-root and use the read-only root filesystem
  where dependencies permit it. Give only `agent-core` a writable data volume.
