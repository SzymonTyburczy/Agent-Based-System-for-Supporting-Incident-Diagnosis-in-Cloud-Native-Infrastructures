...
Start-Process -FilePath "powershell.exe" -ArgumentList '-NoProfile -Command "$host.ui.RawUI.WindowTitle=''OTel-Demo-Store (8080)''; while($true) { kubectl port-forward svc/frontend-proxy 8080:8080 -n otel-demo; Start-Sleep -Seconds 2 }"' -WindowStyle Minimized
Start-Process -FilePath "powershell.exe" -ArgumentList '-NoProfile -Command "$host.ui.RawUI.WindowTitle=''Grafana-UI (8081)''; while($true) { kubectl port-forward svc/prom-stack-grafana 8081:80 -n observability; Start-Sleep -Seconds 2 }"' -WindowStyle Minimized
